...
export function toFormValues(request: ServiceRequestRead): ServiceRequestFormValues {
  return {
    ...
    originPoint:
      request.originPoint ??
      (request.details.type === 'LAUNCH' ? request.details.departurePoint : undefined),
    destinationPoint: request.destinationPoint ?? undefined,
    contactPersonName: request.contactPersonName ?? undefined,
    operationDescription: request.operationDescription ?? undefined,
    ...
    requestingAuthority: request.requestingAuthority ?? undefined,
    supplierInvoiceNo: request.supplierInvoiceNo ?? undefined,
